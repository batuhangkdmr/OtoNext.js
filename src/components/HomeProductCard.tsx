...
export default function HomeProductCard({ product }: Props) {
  ...
  return (
    <div className="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border-2 border-transparent hover:border-primary flex flex-col h-full">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <Link href={productUrl}>
          <h3 className="text-xl font-bold mb-2 text-gray-900 group-hover:text-primary transition-colors line-clamp-2 h-14">
            {product.Name}
          </h3>
        </Link>
        ...
        {(product.CategoryName || product.SubCategoryName) && (
          <div className="flex gap-2 flex-wrap mb-4">
            ...
            {product.SubCategoryName && (
              <span className="inline-block border-2 border-primary text-primary px-3 py-1 rounded-full text-xs font-semibold">
                {product.SubCategoryName}
              </span>
            )}
          </div>
        ...
        <a
          ...
          className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 mt-4 shadow-md hover:shadow-lg"
        >
        ...
        <Link
          href={productUrl}
          className="w-full text-center text-primary font-semibold py-2 hover:underline mt-2"
        >
          Detayları Görüntüle →
        </Link>
      </div>
